...
To compare the out-of-sample predictive performance of our models we used Pareto smoothed importance-sampling leave-one-out cross-validation [@vehtari2015pareto; @vehtari2017practical]. Predictive performance was estimated as the sum of the expected log predictive density ($\widehat{elpd}$) and compared via its difference $\Delta\widehat{elpd}$ between models. We also summarise the comparison between models as the absolute value of the ratio of the difference $\Delta\widehat{elpd}$ and its standard error -- $\mid\frac{\Delta\widehat{elpd}}{\text{SE}}\mid$, hence the *z*-score of the difference between models [@sivula2020uncertainty]. Similar to other cross-validation techniques, the advantage of using leave-one-out cross-validation is that more complicated models -- models with more parameters -- are penalised to prevent overfit.
...
#                "spl2_shift" = "SPL2 (shift + C)", 
                ...
                "c2l1" = "C2L1",
                "gunnexp2" = "GUNNEXP2")),
    Description = case_when(str_detect(model, "M2$") ~ "Single distribution log-Gaussian (M2)",
                            str_detect(model, "M3$") ~ "Single distribution log-Gaussian (unequal variance; M3)",
                            str_detect(model, "M4$") ~ "Two distributions mixture of log-Gaussians (constrained; M4)",
                            str_detect(model, "M5$") ~ "Two distributions mixture of log-Gaussians (unconstrained; M5)"),
...
Model differences for all data sets are shown in Table \ref{tab:loos}. For all data sets we found the same pattern. Overall both two-distributions mixture models provided substantially better fit than any of the single distribution models. Importantly, the constrained two-distributions mixture model rendered a higher predictive performance compared to the unequal variance single distribution model; across datasets the difference $\Delta\widehat{elpd}$ ranged within `r tr_m4` standard errors, indicating a substantially higher predictive performance for the two-distributions model. The unconstrained version of the mixture model rendered a higher predictive performance than the constrained version which did not allow the distribution of short keystroke-intervals to vary across transition locations (and data set specific manipulations); difference in predictive performance was `r tr_m5` standard errors higher for the unconstrained model. Among the single distribution models we found higher predictive performance for the unequal variance model compared to the log-Gaussian model; $\Delta\widehat{elpd}$ was in a range of `r tr_m3` standard errors. The weakest model was the single distribution Gaussian model which had -- compared the single distribution log-Gaussian -- a lower predictive performance in a range of `r tr_m2` standard errors.
...
apa_table(table, 
      caption = "Model comparisons. Models were compared stepwise from the simples to the most complex model. The comparison is indicated along with the description of the more complex model in each comparison. A negative difference in $\\Delta\\widehat{elpd}$ indicates higher predictive performance for the more complex model.  Standard error is shown in parentheses.",
...
```{r mixmodel, fig.cap='Mixture model plot for each transition location. Shown are the distributions for both mixture components (fluent and hesitant transition durations) and their respective weighting. Posterior of CATO data.', fig.height=6}
