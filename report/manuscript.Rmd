...
Interestingly even the youngest sample in our data pool (C2L1)


showed evidence that pauses at before sentence locations were not obligatory and similar to pauses before words. This is interesting because previous research suggested that encoding of subsequent planning units is more separated -- and therefore operates in a serial fashion -- in inexperienced writers [@olive2014toward].
...
Using cross-validation and a simulation we ruled out that the overwhelming statistical support for bi-modal models can be explained in terms of model overfitting. Modelling details can be found in Appendix \ref{simulation}.
...
All plantra results are L2 but not the other studies

to what extent do pauses reflect reading behaviour?
...
(2) dichotomising latencies into pauses and non-pauses discards much of the information that is captured within a keystroke log. 

One strategy for handling the complex distribution of latency from spontaneous text production is to avoid reporting central tendency altogether and to just count latencies that exceed a specific threshold – traditionally set at 2 seconds. This practice, inherited from early research in speech production (see Rochester, 1973 for a review) has been widely adopted by writing researchers, with studies dating back to at least the mid 1990s (Foulin, 1995, 1998). Pause counting has two disadvantages. First, researchers have to make an a priori decision about where to set the pause threshold. Current understanding of the processes that underlie text production does not provide a strong theoretical basis on which to make this decision. So while a 2 s threshold undoubtedly captures an interesting distinction – processing that occurs the range zero to 2 s is very likely to be qualitatively different from processing that takes more than 2 s. However the same could be argued for any threshold between perhaps 250 ms and 10 that a researcher might care to choose (Chenu et al., 2014). Moreover, as we have already discussed, interpretation of latencies depends on location within the text. A sentence-initial pause of 2 s has a very different interpretation from a 2 s pause that occurs before or within a word. A second disadvantage of pause counting is that dichotomizing latencies discards much of the information that is captured within a keystroke log or digitized handwriting trace. We therefore require statistical methods that model the variance across the full range keystroke latencies (or similar measures from handwriting).
...
I just read the abstract of the first one, but the neural oscillations that they describe are occur every 153 ms (6.5 hz). Which is kind of the magic number for the short-duration IKI component.
...
To what extent do pauses reflect reading back. Cite literature from Marks first SPL2 draft on reading during writing.

- Reading is necessary for revision: check if writing goals have been met.
- Planning what to say next: reinstating ideas after interruption of writing process.
- Error evaluation: e.g. spelling, grammar


- we removed transitions that terminate in editiing so lookbacks are not associaed with revisions.

- Adults writing argumentative essays
- @torrance2016reading: 
  - before sentences 45% lookbacks
  - before words 12% lookbacks
  - within word 5% lookbacks 
- 36% sustained reading but mostly less patterned forward and backward saccades between words ["hopping", see also @chukharev2019combined]
- Limited work: @alamargot2010using; @beers2010adolescent; @de2018exploring; @van2010reading
