...
where $i$ iterates over every interkey interval $i \in 1 \ldots N$ with $N$ being the total number of interkey intervals; thus $\text{location[i]}$ is the transition location associated with the $i$^\text{th} keystroke and $\text{participant[i]}$ is the participant associated with the $i$^\text{th} keystroke.
...
This model can be described as in equation \ref{eq:unimodloggaus} in which the distribution $\mathcal{N}()$ was replaced by $\text{log}\mathcal{N}()$.
