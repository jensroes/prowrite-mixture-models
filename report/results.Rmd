...
\text{iki}_{i} & \sim \theta_\text{location[i]} \cdot \text{LogN}(\beta + \delta_\text{location[i]} + \text{participant}_i, \sigma_{e'_\text{location[i]}}^2) + \\
  & (1 - \theta_\text{location[i]}) \cdot \text{LogN}(\beta + \text{participant}_i, \sigma_{e}^2)\\
\text{where: }
		&\delta \sim \text{N}(0,1)\\
...
\end{align}
$$
This models takes into account two sources of participant-specific error: (1) each participant has an individual fluent typing as in the previous models; (2) each participant has in individual hesitation frequency that differs across levels of the categorical predictor.
...
# Datasets in overview

Five dataset with keystroke data from text production were used for analysis. An overview can be found in Table \@ref(tab:datasets).
...
Locations with example from Marks 
...
table <- purrr::map_dfr(files, ~read_csv(.) %>% 
  ...
  select(Model = model, ends_with("CATO"), ends_with("C2L1"), ends_with("LIFT"), ends_with("PLanTra"), ends_with("SPL2"), ends_with("+ C)"))

```


```{r loos, results='asis'}
names(table)[-1] <- rep(c("$\\Delta\\widehat{elpd}$", "$\\widehat{elpd}$"), 6)
...
#                "spl2 (shift + C)" = "SPL2 (L1; shift + C)", 
  ...
  summarise(across(value, 
                   list(mean = mean, 
                        lower = lower, 
                        upper = upper),
                   .names = "{.fn}"), .by = c(data, location, name)) %>% 
...
beta <- filter(ps, name == "beta") %>% 
  ...
  geom_point(aes(colour = data), 
            ...
            position = posd) +
  facet_grid(~name) +
  scale_shape_manual(values = c(1, 4, 7, 8, 9, 11)) +
