...
We start with two uni-modal models -- models that assume that the data come from a single underlying process that can be described as as Gaussian in the first example or a log-Gaussian in the second example. We will compare the models performance.
...
`brms` is using sensible default priors for most model parameters which can be checked using `get_prior`.

```{r}
get_prior(formula = formula, data = data)
```

We want to make sure that `(flat)` is replaced by weakly informative priors because uninformative or flat priors are never a good idea [@mcelreath2016statistical].
...
log_lik(fit_gaus) %>% 
  ...
  pivot_longer(-sample, 
               ...
               values_to = "loglik") %>%
  group_by(obs) %>%
  summarise(M = mean(loglik), var = var(loglik)) %>%
...
or directly calculated using `loo`. The Pareto k diagnostic indicates that there are some observations that are very unusual under the uni-modal Gaussian model.
