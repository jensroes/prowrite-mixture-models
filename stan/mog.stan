...
model {
  ...
  for(n in 1:N){
    ...
    lp_parts[1] = log_theta_s_1[condition[n], subj[n]] + lognormal_lpdf(y[n] | mu, sigma_e[condition[n]]); 
    lp_parts[2] = log_theta_s_2[condition[n], subj[n]] + lognormal_lpdf(y[n] | mu + delta[condition[n]], sigmap_e[condition[n]]); 
