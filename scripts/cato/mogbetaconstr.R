...
# Save model
saveRDS(m, 
        file = "stanout/cato/mogbetaconstr.rda",
        compress = "xz")

#m <- readRDS(file = "stanout/cato/mogbetaconstr.rda")
...
# Extract posterior
ps <- as.matrix(m, c("beta", "beta2", "delta", "theta", "prob")) %>% as_tibble()
...
ps_fin <- ps %>% 
  ...
  mutate(across(c(location, task, group), ~replace_na(., "overall"))) 
  
# Save posterior
write_csv(ps_fin, "stanout/cato/mogbetacontr.csv")
