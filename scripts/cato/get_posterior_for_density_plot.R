...
as.data.frame(m, param) %>%
  ...
  separate(condition, into = c("loc", "xn", "group"), sep = "_") %>%
  mutate(across(loc, ~case_when(str_detect(name, "sigma_diff") & cond_num == 3 ~ "within word", 
                                ...
                                TRUE ~ .)),
         across(c(xn, group), ~ifelse(str_detect(name, "sigma"), NA, .)),
         across(loc, ~replace_na(., "overall"))) %>% 
  filter(is.na(xn) | is.na(group) | xn == "unmasked" & group == "non dyslexic") %>%
  select(param = name, est, loc) %>% 
  # Save posterior
  write_csv("stanout/cato/mog_constr_posterior_for_densityplot.csv")
