...
d <- read_csv("data/lift.csv") %>%
  ...
  mutate(across(ppt, ~as.numeric(factor(.))),
         condition = factor(str_c(genre, topic, sep = "_")),
...
# Save model
saveRDS(m, 
        file = "stanout/lift/mog_withinword.rda",
        compress = "xz")
...
# Extract posterior
ps <- rstan::extract(m, param) %>% as.data.frame()

# Save posterior
write_csv(ps, "stanout/lift/mog_withinword.csv")
