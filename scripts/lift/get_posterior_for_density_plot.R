...
# Read model
m <- readRDS(file = "stanout/lift/mogbetaconstr.rda")
...
as.data.frame(m, param) %>%
  ...
  separate(condition, into = c("loc", "genre", "task"), sep = "_") %>%
  ...
  summarise(across(est, list(mean = mean), .names = "{.col}"), .by = c("name", "loc")) %>% 
  select(param = name, loc, est) %>% 
  # Save posterior
  write_csv("stanout/lift/mog_constr_posterior_for_densityplot.csv")
