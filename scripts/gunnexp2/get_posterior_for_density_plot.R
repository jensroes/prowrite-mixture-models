...
as.data.frame(m, param) %>%
  ...
  separate(condition, into = c("loc", "xn"), sep = "_") %>%
  mutate(across(loc, ~case_when(str_detect(name, "sigma_diff") & cond_num == 3 ~ "within word", 
                                ...
                                TRUE ~ .)),
         across(c(xn), ~ifelse(str_detect(name, "sigma"), NA, .)),
         across(loc, ~replace_na(., "overall"))) %>% 
  filter(is.na(xn) | xn == "unmasked") %>%
  select(param = name, est, loc) %>% 
  # Save posterior
  write_csv("stanout/gunnexp2/mog_constr_posterior_for_densityplot.csv")
